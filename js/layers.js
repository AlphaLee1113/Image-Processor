        ...
        if (currentShadeLayerOp != "no-op") {
            ...
            if (currentShadeLayerOp == "dither" &&
                ...
                for (var i = 0; i < shadeLayer.data.length; i+=4) {
                    if (shadeLayer.data[i] == 255) {  //white
                        shadeLayer.data[i]     = baseLayer.data[i];  //r
                        shadeLayer.data[i + 1] = baseLayer.data[i+1]; //g
                        shadeLayer.data[i + 2] = baseLayer.data[i+2];  //b
                    }
                }
        ...
        if (currentOutlineLayerOp != "no-op") {
            ...
            if (currentOutlineLayerOp == "sobel" &&
                ...
                for (var i = 0; i < outlineLayer.data.length; i+=4) {
                    if (outlineLayer.data[i] == 255 && $("#sobel-flip").prop("checked")==true) {
                        // black line is edge and non edge part is white
                
                        // ... replace the pixel by the shade layer pixel ...
                        outlineLayer.data[i]     = shadeLayer.data[i];  //r
                        outlineLayer.data[i + 1] = shadeLayer.data[i+1]; //g
                        outlineLayer.data[i + 2] = shadeLayer.data[i+2];  //b
                
                    }
                    else if (outlineLayer.data[i] == 0 && $("#sobel-flip").prop("checked")==false) {
                        // white line is edge and non edge part is black
