    ...
    imageproc.kuwahara = function(inputData, outputData, size) {
        ...
        for (var y = 0; y < inputData.height; y++) {
            for (var x = 0; x < inputData.width; x++) {
                ...
                var regionA = regionStat(x - (step/2), y - (step/2), inputData);
                var regionB = regionStat(x +  (step/2), y -  (step/2), inputData);
                var regionC = regionStat(x -  (step/2), y +  (step/2), inputData);
                var regionD = regionStat(x +  (step/2), y +  (step/2), inputData);
